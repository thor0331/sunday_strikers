...
test.describe('Leaderboards', () => {
  test('renders leaderboards and a seeded batter in the batting table', async ({ page, qa }) => {
    watch(page, qa);

    const { players } = await seedCompletedMatch();

    await page.goto('/leaderboards');

    await expect(page.getByRole('heading', { name: 'Leaderboards', exact: true })).toBeVisible();
    await expect(page.getByRole('heading', { name: 'Batting Leaderboard', exact: true })).toBeVisible();
    await expect(page.getByText(players.a[0].display_name, { exact: true })).toBeVisible();

    qa.assertNoPageErrors();
    qa.assertNoNetworkFailures();
  });

  test('switches between batting, bowling, fielding and POTM tabs', async ({ page, qa }) => {
    watch(page, qa);
    ...
    await page.getByRole('button', { name: 'Bowling', exact: true }).click();
    await expect(page.getByRole('heading', { name: 'Bowling Leaderboard', exact: true })).toBeVisible();

    await page.getByRole('button', { name: 'Fielding', exact: true }).click();
    await expect(page.getByRole('heading', { name: 'Fielding Leaderboard', exact: true })).toBeVisible();

    await page.getByRole('button', { name: 'Player of Match', exact: true }).click();
    await expect(page.getByRole('heading', { name: 'Player of the Match', exact: true })).toBeVisible();

    qa.assertNoPageErrors();
    qa.assertNoNetworkFailures();
  });

  test('links to the Hall of Fame', async ({ page, qa }) => {
    watch(page, qa);
