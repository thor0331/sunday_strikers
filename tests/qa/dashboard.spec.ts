...
import { adminDb, type PlayerRow } from './helpers/db';
...
async function findInningsByMatch(matchId: string) {
  const { data, error } = await adminDb().from('innings').select('*').eq('match_id', matchId);
  if (error) throw error;
  return data ?? [];
}

test.describe('Public dashboard', () => {
  test('renders dashboard sections with seeded live and completed matches', async ({ page, qa }) => {
    watch(page, qa);

    const { matchId } = await seedCompletedMatch();
    await seedMatch(makeMatchInput({ name: qaMatchName(), seasonId: null, date: todayIso(), status: 'in_progress' }));
    ...
    await expect(page.getByRole('heading', { name: 'Recent Results', exact: true })).toBeVisible();

    const matchRow = await adminDb().from('matches').select('match_name').eq('id', matchId).single();
    await expect(page.getByRole('link', { name: matchRow.data!.match_name })).toBeVisible();
