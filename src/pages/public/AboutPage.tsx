  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="glass glass-hover rounded-2xl p-6 sm:p-8 text-center relative overflow-hidden">
        {p?.clubBanner && <div className="absolute inset-0 z-0"><img src={p.clubBanner} alt="" className="h-full w-full object-cover opacity-15" onError={(e) => { e.currentTarget.style.display = 'none'; }} /></div>}
        <div className="relative z-10">
          {p?.clubLogo ? (
            <button onClick={() => avatarViewer.open(p.clubLogo!, clubName)} className="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-2xl overflow-hidden shadow-lg ring-4 ring-white/50 hover:scale-105 transition-transform">
              <img src={p.clubLogo} alt={clubName} className="h-full w-full object-cover" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
            </button>
          ) : (
          ...
          <h1 className="text-2xl font-bold tracking-tight text-slate-50 sm:text-3xl">{clubName}</h1>
          {clubMotto && <p className="mt-2 text-sm italic text-slate-400">"{clubMotto}"</p>}
      ...
      {(p?.aboutClub || p?.description) ? (
        <div className="glass glass-hover rounded-2xl p-6">
          <h2 className="mb-3 text-lg font-semibold text-slate-100">About the Club</h2>
          <p className="text-sm leading-relaxed text-slate-300">{p?.aboutClub || p?.description}</p>
        </div>
      ) : null}
      ...
      {p?.clubStory ? (
        <div className="glass glass-hover rounded-2xl p-6">
          <h2 className="mb-3 text-lg font-semibold text-slate-100">Our Story</h2>
          <p className="text-sm leading-relaxed text-slate-300 whitespace-pre-line">{p.clubStory}</p>
        </div>
      ) : null}
      ...
      {(p?.mission || p?.vision) ? (
        <div className="grid gap-4 sm:grid-cols-2">
          {p?.mission ? (
            <div className="glass glass-hover rounded-2xl p-6">
              <h2 className="mb-2 text-lg font-semibold text-slate-100">Mission</h2>
              <p className="text-sm leading-relaxed text-slate-300">{p.mission}</p>
            </div>
          ) : null}
          {p?.vision ? (
            <div className="glass glass-hover rounded-2xl p-6">
              <h2 className="mb-2 text-lg font-semibold text-slate-100">Vision</h2>
              <p className="text-sm leading-relaxed text-slate-300">{p.vision}</p>
            </div>
          ) : null}
        </div>
      ) : null}
      ...
      {p?.features && p.features.length > 0 ? (
        <div className="glass glass-hover rounded-2xl p-6">
          <h2 className="mb-4 text-lg font-semibold text-slate-100">Features</h2>
          <div className="grid gap-3">
            {p.features.map((feature, index) => (
              <div key={index} className="flex items-start gap-3 rounded-xl border border-white/10 bg-white/[0.04] p-4 transition-all hover:border-teal-400/30">
      ...
      {(p?.contactEmail || p?.instagram || p?.facebook) ? (
        <div className="glass glass-hover rounded-2xl p-6">
          <h2 className="mb-3 text-lg font-semibold text-slate-100">Connect With Us</h2>
          <div className="flex flex-wrap gap-3">
            {p?.contactEmail && <a href={`mailto:${p.contactEmail}`} className="flex items-center gap-2 rounded-lg border border-white/10 bg-white/10 px-4 py-2 text-sm text-slate-300 hover:border-teal-400/40 hover:text-teal-300 transition-all"><Mail className="w-4 h-4" />Email</a>}
            {p?.instagram && <a href={p.instagram} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 rounded-lg border border-white/10 bg-white/10 px-4 py-2 text-sm text-slate-300 hover:border-teal-400/40 hover:text-teal-300 transition-all"><Instagram className="w-4 h-4" />Instagram</a>}
            {p?.facebook && <a href={p.facebook} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 rounded-lg border border-white/10 bg-white/10 px-4 py-2 text-sm text-slate-300 hover:border-teal-400/40 hover:text-teal-300 transition-all"><Facebook className="w-4 h-4" />Facebook</a>}
          </div>
        </div>
      ) : null}
      ...
      {developers.length > 0 ? (
        <div className="glass glass-hover rounded-2xl p-6">
          <h2 className="mb-4 text-lg font-semibold text-slate-100">Developers</h2>
          <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div key={i} className="flex items-center gap-4 rounded-xl border border-white/10 bg-white/[0.04] p-4">
                {dev.photoUrl ? (
                  <button onClick={() => avatarViewer.open(dev.photoUrl!, dev.name, dev.role)} className="shrink-0 h-14 w-14 overflow-hidden rounded-full border-2 border-white shadow-md hover:scale-105 transition-transform">
                    <img src={dev.photoUrl} alt={dev.name} className="h-full w-full object-cover" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
                  </button>
                ) : (
                ...
                <div className="min-w-0 flex-1">
                  <p className="font-semibold text-slate-100 truncate">{dev.name}</p>
                  {dev.role && <p className="text-xs text-slate-400">{dev.role}</p>}
                  <div className="mt-1.5 flex flex-wrap gap-2">
                    {dev.github && <a href={dev.github} target="_blank" rel="noopener noreferrer" className="text-xs text-slate-400 hover:text-teal-300 transition-colors"><Globe className="w-3 h-3 inline" /> GitHub</a>}
                    {dev.linkedin && <a href={dev.linkedin} target="_blank" rel="noopener noreferrer" className="text-xs text-slate-400 hover:text-teal-300 transition-colors"><Globe className="w-3 h-3 inline" /> LinkedIn</a>}
                    {dev.website && <a href={dev.website} target="_blank" rel="noopener noreferrer" className="text-xs text-slate-400 hover:text-teal-300 transition-colors"><Globe className="w-3 h-3 inline" /> Website</a>}
                    {dev.email && <a href={`mailto:${dev.email}`} className="text-xs text-slate-400 hover:text-teal-300 transition-colors"><Mail className="w-3 h-3 inline" /> Email</a>}
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="glass rounded-2xl p-6 text-center">
          <h2 className="mb-3 text-lg font-semibold text-slate-100">Developers</h2>
...
function StatCard({ icon, label, value, gradient }: { icon: React.ReactNode; label: string; value: number; gradient: string }) {
  return (
    <GlassCard variant="light" className="p-3 text-center">
      ...
      <p className="text-lg font-bold text-slate-100 tabular-nums">{value.toLocaleString()}</p>
