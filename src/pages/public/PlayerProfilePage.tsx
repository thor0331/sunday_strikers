...
export function PlayerProfilePage() {
  ...
  if (!player) return <div className="flex min-h-[50vh] items-center justify-center"><p className="text-sm text-slate-500">Player not found.</p></div>;
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => navigate(-1)} className="flex h-10 w-10 items-center justify-center rounded-full hover:bg-slate-100 transition-colors"><ArrowLeft className="w-5 h-5 text-slate-600" /></button>
        <h1 className="text-lg font-bold text-slate-800">Player Profile</h1>
      </div>
      ...
      <GlassCard variant="strong" className="p-6 text-center">
        {player.photo_url ? (
          <button onClick={() => avatarViewer.open(player.photo_url!, player.display_name)} className="mx-auto mb-3 h-24 w-24 overflow-full rounded-full border-4 border-white shadow-lg hover:scale-105 transition-transform">
            <img src={player.photo_url} alt={player.display_name} className="h-full w-full object-cover" />
          </button>
        ) : (
        ...
        <h2 className="text-xl font-bold text-slate-800">{player.display_name}</h2>
        <div className="mt-2 flex flex-wrap justify-center gap-2">
          {player.batting_style && <span className="rounded-full bg-teal-100 px-3 py-0.5 text-xs font-medium text-teal-700">{player.batting_style}</span>}
          {player.bowling_style && <span className="rounded-full bg-blue-100 px-3 py-0.5 text-xs font-medium text-blue-700">{player.bowling_style}</span>}
        </div>
      </GlassCard>
        ...
        <GlassCard variant="light" className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-slate-700">{captainWins} wins in {captainMatches} matches ({winPercentage}%)</p>
            </div>
          </div>
        </GlassCard>
      )}
      ...
      {aggregatedStats && (
        <GlassCard variant="light" className="p-4">
          <h3 className="text-sm font-bold text-slate-700 mb-3">Career Statistics</h3>
      ...
      {streaks.length > 0 && (
        <GlassCard variant="light" className="p-4">
          <h3 className="text-sm font-bold text-slate-700 mb-3">Active Streaks</h3>
          <div className="flex flex-wrap gap-2">
            {streaks.map((s, i) => (
              <span key={i} className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-3 py-1 text-xs font-medium text-amber-700">
      ...
      {recentMatches.length > 0 && (
        <GlassCard variant="light" className="p-4">
          <h3 className="text-sm font-bold text-slate-700 mb-3">Recent Matches</h3>
          <div className="space-y-2">
            {recentMatches.map((m) => (
              <button key={m.id} onClick={() => navigate(`/matches/${m.id}`)} className="w-full text-left rounded-lg border border-slate-100 bg-white/60 p-3 hover:border-teal-200 transition-all">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-slate-700 truncate">{m.match_name}</p>
                    <p className="text-xs text-slate-400">{m.match_date}</p>
                  </div>
                  {m.result_text && <span className="text-xs text-slate-500 truncate max-w-[40%] text-right">{m.result_text}</span>}
                </div>
              </button>
            ))}
          </div>
      ...
      {inningsHistory.length > 1 && (
        <GlassCard variant="light" className="p-4">
          <h3 className="text-sm font-bold text-slate-700 mb-3">Performance Trend</h3>
          <div className="flex items-end gap-1 h-24">
            {inningsHistory.map((inn, i) => {
              const maxRuns = Math.max(...inningsHistory.map((h) => h.runs), 1);
              const height = (inn.runs / maxRuns) * 100;
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <span className="text-[9px] text-slate-400 tabular-nums">{inn.runs}</span>
                  <div className={`w-full rounded-t transition-all ${inn.isOut ? 'bg-red-300' : 'bg-teal-400'}`} style={{ height: `${Math.max(height, 4)}%` }} />
                </div>
              );
            })}
          </div>
