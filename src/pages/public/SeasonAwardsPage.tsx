      ...
      <PagePanel title="Season Awards">
        {awards.length === 0 ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {awards.map((award) => (
              <div key={award.category} className="rounded-xl border border-white/10 bg-white/[0.03] p-4 transition-all">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <Link to={`/players/${award.playerId}`} className="font-bold text-slate-100 hover:text-teal-300 transition-colors truncate">
                        {award.playerName}
                      </Link>
                    </div>
                    <p className="text-sm text-teal-300 font-semibold mt-1">{award.value}</p>
                  </div>
                </div>
              </div>
            ))}
