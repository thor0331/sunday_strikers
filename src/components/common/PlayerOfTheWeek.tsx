  ...
  return (
    <GlassCard variant="light" hover className="p-4 overflow-hidden relative">
      ...
      <div className="flex items-center gap-2 mb-3 relative">
        <div className="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-amber-400/10 text-amber-300">
          <Flame className="w-4 h-4" />
        </div>
        <span className="text-[11px] font-bold uppercase tracking-wider text-amber-300">Player of the Week</span>
      </div>
      <div className="flex items-center gap-3 relative">
        ...
        <div className="min-w-0">
          <p className="font-bold text-slate-100 truncate text-base">{potw.player.display_name}</p>
...
function StatBadge({ label, value, color, highlight }: { label: string; value: number; color: string; highlight?: boolean }) {
  const colors: Record<string, { bg: string; text: string }> = {
    ...
    blue: { bg: 'bg-sky-400/10', text: 'text-sky-300' },
